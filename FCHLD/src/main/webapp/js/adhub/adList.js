	...
	function fncSearch(){
		...
		{		
			...
			$("#form").attr("action","/mktp/adhub/getListAd2.do");		
			$("#form").submit();
		}
	}
	...
	function paginationClick( pageNo ) {
		...
		$("form[name=form]").attr("action", "/mktp/adhub/getListAd2.do");
	...
	function fncAdDetail(obj,stat,pushYn,store){
		...
		if(pushYn == "Y"){
			if(state == "A" || state == "C"){
		 		$("#form").attr("action","/mktp/adhub/adGoodPushD.do");		
		 		$("#form").submit();			
			}else if(state == "R" || state == "W"){
		 		$("#form").attr("action","/mktp/adhub/adGoodPushU.do");		
		 		$("#form").submit();
			}			
		}else{
			if(state == "A" || state == "C"){
		 		$("#form").attr("action","/mktp/adhub/adGoodD.do");		
		 		$("#form").submit();			
			}else if(state == "R" || state == "W"){
		 		$("#form").attr("action","/mktp/adhub/adGoodU.do");		
		 		$("#form").submit();
			}			
		}
	...
	function fncNew(){	
		checkBoxValue();		
		$("#form").attr("action","/mktp/adhub/adGoodR.do");		
		$("#form").submit();				
	}
	...
	function fncNewPush(){	
		checkBoxValue();		
		$("#form").attr("action","/mktp/adhub/adGoodPushR.do");		
		$("#form").submit();				
	}
