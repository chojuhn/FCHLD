	var checkWords = new Array( "javascript","eval","onmousewheel","onactive","innerHTML",
								"expression","charset","ondataavailable","oncut","onfocusout",
								"applet","document","onafteripudate","onclick","onkeyup",
								"meta","string","onmousedown","onchage","onkeypress",
								"xml","create","onbeforeactivate","onbeforecut","onload",
								"blink","append","onbeforecopy","ondbclick","onbounce",
								"link","binding","onbeforedeactivate","ondeactivate","onmouseenter",
								"style","alert","ondatasetchaged","ondrag","onmouseout",
								"script","msgbox","cnbeforeprint","ondragend","onmouseover",
								"embed","refresh","cnbeforepaste","ondragenter","onsubmit",
								"object","void","onbeforeeditfocus","ondragleave","onmouseend",
								"iframe","cookie","onbeforeuload","ondragover","onresizestart",
								"frame","Href","onbeforeupdate","ondragstart","onuload",
								"frameset","onpaste","onpropertychange","ondrop","onselectstart",
								"ilayer","onresize","ondatasetcomplete","onerror","onreset",
								"layer","onselect","oncellchage","onfinish","onmove",
								"bgsound","base","onlayoutcomplete","onfocus","onstop",
								"title","onblur","onselectionchange","vbscript","onrowexit",
								"onbefore","onstart","onrowsinserted","onkeydown","onerrorupdate",
								"onmouseup","onfocusin","oncontrolselected","onrowsdelete","onfilterchage",
								"onrowenter","onhelp","onreadystatechange","onmouseleave","onlosecapture",
								"oncontextmenu","onmousemove");

	function loading(mvmnTlcnCmpy,tgtCusGndr,tgtCusAge,blbrExpsYn,pupExpsYn,pntAcmtDvs,pntAcmtRto,pntAcmtAmt,stlmMthd,stlmMthd2,randingPageYn,ptuCyc,advrLinkUrl,advrTyp,resultMessage){
		...
		if(stlmMthd == 'M' || stlmMthd == 'W'){
			$("#stlmMthd2").hide();
			$("#stlmMthd").val(stlmMthd);
		}else if(stlmMthd == 'D'){
			$("#stlmMthd2").show();
			$("#stlmMthd").val(stlmMthd);
			$("#stlmMthd2").val(stlmMthd2);
		}
		...
		//광고 url
		if(advrLinkUrl == "" || advrLinkUrl == "http://"){
			$("#advrLinkUrl").val("http://");
		}else{
			$("#advrLinkUrl").val(advrLinkUrl);
		}
	...
	function checkText(){
		for(var i = 0 ; i < $('input[type="text"]').length; i++){
			...
			var num = 0;
			var find = -1;
			var chkContext = $('input[type="text"]:eq(' + i + ')').val();
			
			while((num < checkWords.length) && (find < 0)){
				
				find = chkContext.indexOf(checkWords[num]);
				
				if(find >= 0){
					alert("{"+ checkWords[num] +"}는 입력할 수 없는 단어입니다");
	...
	function btnInitClick()
	{
		...
		$("#form2").attr("action","/mktp/adhub/getListAdStore2.do");		
		$("#form2").submit();						
	}
	...
	function btnSave() {
		...
		var datepicker01 = $("#datepicker01").val();
		var datepicker02 = $("#datepicker02").val();
//		alert("datepicker01 : " + datepicker01 + " datepicker02 : "+datepicker02);
		var sttDt  = datepicker01.replace(/-/gi,"");
		var closDt = datepicker02.replace(/-/gi,"");
...
//		alert("mvmnTlcnCmpy :"+mvmnTlcnCmpy + "  tgtCusGndr :"+tgtCusGndr + "  tgtCusAge :"+tgtCusAge);
//		alert("blbrExpsYn :"+blbrExpsYn + " pupExpsYn :"+pupExpsYn );
		...
		if ( confirm("저장을 하시겠습니까?") ) {
			$("#form").attr("action","/mktp/adhub/insertAdStore.do");		
			$("#form").submit();
		}
	}
