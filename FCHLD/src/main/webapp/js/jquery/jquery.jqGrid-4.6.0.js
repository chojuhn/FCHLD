		...
		formatter = function (rowId, cellval , colpos, rwdat, _act){
			...
			if(cm.formatter !== undefined) {
				...
				if($.isFunction( cm.formatter ) ) {
					...
					// 원본 소스 변경 2014-03-06
	                if( _act == "addrowdata" || _act == "edit" ){
	                    v = cellValNonEncode(cellval);
	                } else {
	                    v = cellVal(cellval);
	                }				    
				}
		...
		this.each(function() {
			...
			if( $t.p.altRows === true && success ) {
				...
				$($t.rows).each(function(i){
				    if( $(this).hasClass("jqgfirstrow") ) chkFirstRow = 1; 
					if( (i+chkFirstRow) % 2 === 1) { $(this).addClass(cn); }
			...
			this.each(function() {
				...
				while(k < datalen) {
					...
					for(i = gi+si+ni; i < t.p.colModel.length;i++){
						...
						lcdata[nm] = data[nm];
						// 원본 소스 변경 2014-03-06
						v = t.formatter( rowid, $.jgrid.getAccessor(data,nm), i, data, "addrowdata" );
				...
				if( t.p.altRows === true && !aradd) {
				    ...
				    var chkFirstRow = 0;
					if (pos === "last") {
					    try{ if( $(t.rows[0]).hasClass("jqgfirstrow") ) chkFirstRow = 1; } catch(e) {}
						if ((t.rows.length-1+chkFirstRow)%2 === 1) {$(t.rows[t.rows.length-1]).addClass(cn);}
					} else {
						$(t.rows).each(function(i){
		                    if( $(this).hasClass("jqgfirstrow") ) chkFirstRow = 1;
							if((i+chkFirstRow) % 2 ===1) { $(this).addClass(cn); }
			...
			if(fr >= 0) {
				...
				$.each($t.p.colModel, function(){
					if(this.editable === true && $t.p.savedRow[fr].hasOwnProperty(this.name)) {
					    // 원본 소스 변경 2014-03-07 : 저장되었던 로우데이터를 엔코딩해서 화면에 출력해준다.
						var savedData = $t.p.savedRow[fr][this.name];
						if( $t.p.autoencode ){ savedData = $.jgrid.htmlEncode(savedData); } 
					    ares[this.name] = savedData;
					}
				});
