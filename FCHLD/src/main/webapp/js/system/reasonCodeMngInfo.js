...
function btnSearchClick() {
	...
	if ( result )
	{
		...
		$("form[name=form]").attr("action", "/mktp/system/reasonCodeMngInfo.do");
...
function btnExcelClick() {
	...
	$("form[name=form]").attr("action", "/mktp/system/excelReasonCodeMngInfo.do");
	$("form[name=form]").submit();
}


function btnAddKeyClick() {
	$("form[name=form]").attr("action", "/mktp/system/reasonCodeInsert.do");
	$("form[name=form]").submit();
}

function goDetail(cncnRsnCd) {
	$("input[name=cncnRsnCd]").val(cncnRsnCd);
	$("form[name=form]").attr("action", "/mktp/system/reasonCodeUpdate.do");
	$("form[name=form]").submit();
}
...
function showDetail( mbrMngNo ) {
	...
	$("form[name=form]").attr("action", "/mktp/member/system/reasonCodeMngInfoDetail.do");
	$("form[name=form]").submit();
}
...
//		};
//		ComUtil.popupPost("/mktp/system/popup/updateReasonCodeMngInfo.do", "800", "320", "viewGroupInfo", data);
//
//	}
//}
//
function btnDeleteKeyClick() {
	...
	if (checkedAuthKeySeq.length == 0) {
		alert("삭제할 탈회사유코드를 선택하세요.");
		return;
	}
	
	if (!confirm("선택한 탈회사유코드를 삭제하시겠습니까?")) {
		return;
	}
	
	$("#authKeySeqList").val(checkedAuthKeySeq.join(","));
	$("#form").attr("action", "/mktp/system/deleteReasonCodeInfo.do");
	$("#form").submit();
}