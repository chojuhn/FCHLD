...
function btnSearchClick() {
	...
	if ( result )
	{
		...
		if(ComUtil.removeMask($("input[name=searchInqrDtFrom]").val()) > ComUtil.removeMask($("input[name=searchInqrDtTo]").val())) {
			...
			$("input[name=searchInqrDtFrom]").focus();
			return;
		}
		
		$("input[name=currentPage]").val(1);
		$("input[name=pageSize]").val($("select[name=searchPageSize]").val());
		$("form[name=form1]").attr("action", "/mktp/member/popup/affiliateCardPaymentInfo.do");
...
function btnExcelClick() {
	...
	$("form[name=form1]").attr("action", "/mktp/member/popup/excelAffiliateCardPaymentInfo.do");
	$("form[name=form1]").submit();
}
