...
/* 엑셀 팝업화면 오픈 */
function fnOpenCsvPup() {
    fnComOpenPopup("<c:url value='/sample/sampleCsvPopup.do'/>", "height=670", "fnCsvCallBack", null, "");
}

/* 팝업에서 호출될 callback 함수 */
function fnCsvCallBack() {
    alert("callback");
}

/* 엑셀다운로드 버튼 클릭 ============================================================================*/
function fnCsvDownLoad() {
     $("input[name=currentPage]").val(1);
     $("input[name=pageSize]").val($("select[name=searchPageSize]").val());

     $("#form1").attr("action", "<c:url value='/sample/sampleCsvDownload.do'/>").submit();
}
</script>
        ...
        <div class="Lwrapper">
            <div class="LblockPageSubtitle">
                ...
                <div id="LblockSubbutton01" class="LblockSubbutton">
                    <span class="LbtnIcon"><a href="javascript:;" id="btnCsv" name="btnCsv" onclick="fnCsvDownLoad();"><img src="<c:url value='/images/icon_down.gif'/>" alt="CSV다운로드"/>CSV다운로드</a></span>
                </div>
            </div>
